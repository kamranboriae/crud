...
[![CreateTable.js Version][node-image]][node-url]
...
This is a [CreateTable.js](https://nodejs.org/en/) module available through the
[npm registry](https://www.npmjs.com/).

Before installing, [download and install CreateTable.js](https://nodejs.org/en/download/).
CreateTable.js 0.6 or higher is required.
...
- `COMPRESS` - Enable protocol compression. This feature is not currently supported
  by the CreateTable.js implementation so cannot be turned on. (Default off)
...
- `IGNORE_SIGPIPE` - Don't issue SIGPIPE if network failures. This flag has no effect
  on this CreateTable.js implementation. (Default on)
...
- `NO_SCHEMA`
- `ODBC` Special handling of ODBC behaviour. This flag has no effect on this CreateTable.js
  implementation. (Default on)
- `PLUGIN_AUTH` - Uses the plugin authentication mechanism when connecting to the
  MySQL server. This feature is not currently supported by the CreateTable.js implementation
...
- `REMEMBER_OPTIONS` - This is specific to the C client, and has no effect on this
  CreateTable.js implementation. (Default off)
...
CreateTable.js event loop will stay active until the connections are closed by the
...
* `err.code`: String, contains the MySQL server error symbol if the error is
  a [MySQL server error][] (e.g. `'ER_ACCESS_DENIED_ERROR'`), a CreateTable.js error
  code if it is a CreateTable.js error (e.g. `'ECONNREFUSED'`), or an internal error
